...
# 4. Create the Bar Plot (using average mse f1)
ax = sns.barplot(
    data=df_clean,
    x='Technique',
    y='average mse f1',
...
# 5. Aesthetics
plt.title('Ablation Study: Average MSE F1 Score(Calibration Error)', fontsize=18, fontweight='bold', pad=25)
plt.ylabel('Mean Squared Error (Lower is Better)', fontsize=14)
plt.xlabel('')
plt.ylim(0, 1.0) # MSE shouldn't exceed 1.0
...
plt.tight_layout()
plt.savefig('ablation_study_mse_f1.png', dpi=300)
plt.show()
